                ...
                for move in WorkerAction.moves():
                    ...
                    if new_distance < old_distance:
                        ...
                        if self.cal_cell_tree_num(
                                cal_new_pos_cell(collector.cell, move)) >= 2 and collector.carbon > 50:
                            continue
                        if self.cal_cell_tree_num(cal_new_pos_cell(collector.cell, move))>=1 and collector.carbon>80:
                            continue
...
class PlanterAct(AgentBase):
    ...
    def _target_plan(self, planter: Planter, carbon_sort_dict: Dict, ours_info, oppo_info):
        ...
        for _cell, _carbon_sum in carbon_sort_dict.items():
            if self.expendable_id==planter.id and _cell.position!=center \
                ...
                target_preference_score = 100000 #+_carbon_sum + planter_to_cell_distance * (-20) #+ 10*len(oppo_info[0].collectors)
                if target_preference_score > max_score:
                    max_score = target_preference_score
                    max_score_cell = _cell
    ...
    def move(self, ours_info, oppo_info, **kwargs):
        ...
        for planter in ours_info.planters:
            ...
            if min_dis==planter_to_oppo_dis and len(oppo_info[0].collectors)>2:
        ...
        for planter in ours_info.planters:
            ...
            if planter.position == self.planter_target[planter.id].position:
                ...
                if ((money * 1.3 < carbon or money<50) and (kwargs['step']<290)) or self.expendable_id==planter.id:
...
class RecruiterAct(AgentBase):
    ...
    def action(self, ours_info, oppo_info, **kwargs):
        ...
        if not ours_info.recrtCenters[0].cell.position in overall_plan:
            ...
            if len(ours_info.planters)<1 and len(ours_info.collectors)>3:
                store_dict[ours_info.recrtCenters[0].id] = RecrtCenterAction.RECPLANTER.name

            if (ours_info.recrtCenters[0].cell.collector is not None and
                    ours_info.recrtCenters[0].cell.collector.player_id != ours_info.recrtCenters[0].player_id):
                store_dict[ours_info.recrtCenters[0].id] = RecrtCenterAction.RECCOLLECTOR.name
            '''
