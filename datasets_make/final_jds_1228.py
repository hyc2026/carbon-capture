...
def cal_obs(current_obs, previous_obs):
    ...
    for cell_id in current_obs.cells:
        ...
        if (cell_id in viewed) and abs(gl_cells[cell_id].carbon - current_obs.cells[cell_id].carbon) > 0.3 and current_obs.cells[cell_id].carbon>0:
            ha = 1
    ...
    for collector in ours_info.collectors:
        ...
        if pos in viewed:
            if abs(gl_cells[pos].carbon - current_obs.cells[pos].carbon)>0.3:
                ha = 1
...
class MyPolicy:
    ...
    def take_action(self, observation, configuration):
        ...
        viewed_index = []
        for cell_id in my_viewed:
            #if new_obs.cells[cell_id].carbon!=0 and new_obs.cells[cell_id]._carbon != my_obs[cell_id].carbon:
            #    ha = 0
            new_obs.cells[cell_id]._carbon = my_obs[cell_id].carbon
            viewed_index.append(new_obs.cells[cell_id].position)
        ...
        masked_map = np.zeros((15, 15))
        for p in my_viewed:
            masked_map[p[0]][p[1]] = 1
        masked_map = masked_map.tolist()
